...
const getMentionPartSuggestionKeywordsNormal = (
  parts: Part[],
  plainText: string,
  selection: Position,
  nomalTriggers: NomalPartType[] | undefined,
  handleSetIstracking: any,
): {value: string; nomalTrigger: MentionPartType} => {
  const keywordByTrigger: {[trigger: string]: string | undefined} = {};
  let results: {value: string; nomalTrigger: MentionPartType} = {
    value: '',
    nomalTrigger: {
      trigger: '',
  ...
  nomalTriggers?.filter(isMentionPartType).forEach(({trigger, mentionType}) => {
    ...
    for (let cursor = selection.end - 1; cursor >= triggerIndex; cursor -= 1) {
      plainText[cursor - 1] === trigger && handleSetIstracking(true);
    ...
    results.value = plainText.substring(triggerIndex + 1, selection.end);
    results.nomalTrigger = mentionType;
  });
      ...
      concatMap((val: any) => {
        return from([...val]).pipe(
          map((item: any) => {
            ...
            const [empty, fullMatch, trigger, name, id] =
              item.split(mentionRegEx);
